...
def get_species_counts(chain_sequence,desired_dw=0.0001,max_conc=0.5):
    ...
    for j in range(len(C0)):
        ...
        C_TMA = 0.0238*C_SOL*(molweights[1][-1]/(molweights[1][-1]+molweights[2][-1]))
        n_TMA = round(C_TMA*tot_w/74.14)
        C_OH = 0.0238*C_SOL*(molweights[2][-1]/(molweights[1][-1]+molweights[2][-1]))
...
def main():
    chain_sequences = [str("A"*i)+str("G"*(5-i)) for i in range(1,5)]
    chain_sequences = ["GGGGAGGGGAGGGGAGGGGA","GGGGGGGGAGGGGAGGGGAA","GGGGGGGGGGGGAGGGGAAA","GGGGGGGGGGGGGGGAAAAA"]
    chain_sequences = ["GGGGG"]
    first_spindoals = []
    output_data = []
