...
def get_species_counts(chain_sequence,dw=1e-6,max_conc=0.25,tmah_conc=0.0238,precision=0.001):
    ...
    beadtypes = [['HOH', 0.75, 0], ['ARO', 0.75, 0], ['GLU', 0.75, -1], ['TMA', 0.75, 1], ['OH', 0.75, -1]]
    ...
    for j in range(len(C0)):
        ...
        n_waters = round(C_W*tot_w/18.01528) # before cluster
...
def main():
    ...
    g_arr = [0, 1, 3, 4, 5, 6, 8, 9, 10, 11, 13, 14, 15, 16, 18, 19]
    gp_all = choose_even_cuts(g_arr,10)
    res1 = []
    for i in range(20):
        if i in gp_all:
            res1.append("Gp")
            ...
            res1.append("A")
    print("-".join(res1))
    gp_all = choose_even_cuts(g_arr,9)
    res2 = []
    for i in range(20):
        if i in gp_all:
            res2.append("Gp")
    ...
    #print(''.join(res2))
    #exit()
    #chain_sequences = [str("A"*i)+str("G"*(20-i)) for i in range(1,20)]
    chain_sequences = [str("A"*20)+str("G"*(30))]
    ...
    for chain_sequence in chain_sequences:
        ...
        aro_comp_fraction = sum([1 if i == 'A' else 0 for i in chain_sequence])/len(chain_sequence)
        glu_comp_fraction = 1-aro_comp_fraction
        DS = get_species_counts(chain_sequence,dw=1e-7,max_conc=0.2,precision=0.0005)
        #print(DS)
        #print(DS.keys())
        
        # recompute, accounting for water clustering (6 HOH per bead, 5 HOH per 1 OH-/H3O+)
        n_h2o_per_cluster = 6
        ...
        for key, species_counts_data in DS.items():
            ...
            if len(key) == 2 and key[0] == 1.0: # fully deprotonated case
                ...
                RS = RPA_OBJECTS[key]
                RS.ff_file = './CG_FF_with_clustered_water/cg_ff_GLUd_only.dat'
                RS.out = 'ARO_GLU'
                RS.bondtype = 'offset'
                RS.r_max = 10.
                RS.n_r = 10000
                ...
                RS.n_k = 10000
                n = 1000
                RS.beadtypes = [['HOH', 0.75, 0], ['ARO', 0.75, 0], ['GLUd', 0.75, -1], ['TMA', 0.75, 1], ['OH', 0.75, -1]]
                RS.moltypes = [['HOH',[0]], ['TMA',[3]], ['OH',[4]], ['Chain',chain_mol_beads]]
                RS.molbonds = [['HOH',[]], ['TMA',[]], ['OH',[]], ['Chain',[[idx,idx+1] for idx in range(chain_len-1)]]]
                RS.molweights = [['HOH',18.01528*6],['TMA',74.14],['OH',17.007+5*18.01528],['Chain',aro_comp_fraction*chain_len*161.2004+(1-aro_comp_fraction)*chain_len*128.1060]] # chain is 5 ARO and 5 GLU
                chain_vol = (1-aro_comp_fraction)*chain_len*0.130306509511330 + (aro_comp_fraction*chain_len)*0.22259864959526
                RS.molvols = [['HOH',0.03001009271548933*6],['TMA',0.11820332057803033],['OH',0.03001009271548933*6],['Chain',chain_vol]]
                RS.comps = []
                for data_col in res:
                ...
                RS = RPA_OBJECTS[key]
                RS.ff_file = './CG_FF_with_clustered_water/cg_ff_GLUd_only.dat'
                #RS.ff_file = 'prev_ff/cg_ff_GLUd_only_prev.dat'
                RS.out = 'ARO_GLU'
                RS.bondtype = 'offset'
                RS.r_max = 10.
                RS.n_r = 10000
                ...
                RS.n_k = 10000
                n = 1000
                RS.beadtypes = [['HOH', 0.75, 0], ['ARO', 0.75, 0], ['GLUp', 0.75, -1], ['TMA', 0.75, 1], ['OH', 0.75, -1]]
                RS.moltypes = [['HOH',[0]], ['TMA',[3]], ['OH',[4]], ['Chain',chain_mol_beads]]
                RS.molbonds = [['HOH',[]], ['TMA',[]], ['OH',[]], ['Chain',[[idx,idx+1] for idx in range(chain_len-1)]]]
                RS.molweights = [['HOH',18.01528*6],['TMA',74.14],['OH',17.007+18.01528*5],['Chain',aro_comp_fraction*chain_len*161.2004+(1-aro_comp_fraction)*chain_len*129.114]] # chain is 5 ARO and 5 GLU
                ...
                RPA_OBJECTS[key] = RPA.RPAsystem()
                RS = RPA_OBJECTS[key]
                RS.ff_file = './CG_FF_with_clustered_water/cg_ff_GLUd_GLUp.dat'
                RS.out = 'ARO_GLU'
                RS.bondtype = 'offset'
                RS.r_max = 10.
                RS.n_r = 10000
                ...
                n = 1000
                if has_h3o:
                    RS.beadtypes = [['HOH', 0.75, 0], ['ARO', 0.75, 0], ['GLUd', 0.75, -1],['GLUp', 0.75, 0], ['TMA', 0.75, 1], ['H3O', 0.75, 1]]
                    ...
                    RS.molweights = [['HOH',18.01528*6],['TMA',74.14],['H3O',19.0232+18.01528*5],['Chain1',aro_comp_fraction*chain_len*161.2004+num_deprot_monos1*128.1060+num_prot_monos1*129.114],['Chain2',aro_comp_fraction*chain_len*161.2004+num_deprot_monos2*128.1060+num_prot_monos2*129.114]] # chain is 5 ARO and 5 GLU
                    #RS.molvols = [['HOH',0.029978],['TMA',0.06753],['H3O',0.029978],['Chain1',0.135253*chain_len],['Chain2',0.135253*chain_len]]
                    chain_vol = (1-aro_comp_fraction)*chain_len*0.130306509511330 + (aro_comp_fraction*chain_len)*0.22259864959526
                    RS.molvols = [['HOH',0.03001009271548933*6],['TMA',0.11820332057803033],['H3O',0.03001009271548933*6],['Chain1',chain_vol],['Chain2',chain_vol]]
                    RS.comps = []
                    for data_col in res:
                    ...
                    RS.beadtypes = [['HOH', 0.75, 0], ['ARO', 0.75, 0], ['GLUd', 0.75, -1],['GLUp', 0.75, 0], ['TMA', 0.75, 1]]
                    ...
                    RS.molweights = [['HOH',18.01528*6],['TMA',74.14],['Chain1',aro_comp_fraction*chain_len*161.2004+num_deprot_monos1*128.1060+num_prot_monos1*129.114],['Chain2',aro_comp_fraction*chain_len*161.2004+num_deprot_monos2*128.1060+num_prot_monos2*129.114]] # chain is 5 ARO and 5 GLU
                    chain_vol = (1-aro_comp_fraction)*chain_len*0.130306509511330 + (aro_comp_fraction*chain_len)*0.22259864959526
                    RS.molvols = [['HOH',0.03001009271548933*6],['TMA',0.11820332057803033],['Chain1',chain_vol],['Chain2',chain_vol]]
                    RS.comps = []
                    for data_col in res:
                        n_chains1, n_chains2, nHOH, nTMA, nOH, nH3O = data_col
        ...
        with open("RPA_clustered_water.txt", "w") as f: # [C_W,C_TMA,C_OH,C_CHAIN,C_H3O]
        ...
        stability_all = []
        polymer_concs = []
        with open("RPA_clustered_water.txt","r") as f:
    ...
    print(output_data)
    np.savetxt("blocklength_fA20.csv", output_data, delimiter=",")
